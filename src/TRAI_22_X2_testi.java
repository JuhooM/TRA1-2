 ...
 */
public class TRAI_22_X2_testi {

    static Random rnd = new Random();

    static TRAI_22_X2 testattava = new TRAI_22_X2_juhomati(); /* <-- Oma tunnus tÃ¤hÃ¤n */
    ...
    static boolean testaaTulosX2(int n1, int n2, int min1, int min2,
        ...
        ArrayList<Integer> tul = testattava.kasvavienLeikkaus(A, B);

        // tulostetaan tulos
        if ((tul.size() < 20 && print > 1) || print > 5)
            System.out.println("Leikkaus   : " + tul);

        boolean ok = true;

        ok &= onkoJarjestyksessa(tul, print);

        ok &= !onkoDuplikaatteja(tul, print);
        ...
        ok &= vertaa(tul, leikkausJoukoilla(cA, cB), print);
     ...
     * JOUKON KUVAUKSEN KÃ„YTTÃ„MINEN. EIKÃ„ TÃ„MÃ„ OLE EDES HYVÃ„ MALLI KOSKA TÃ„MÃ„ EI OLE TEHOKKAIN
     ...
     */
    static public ArrayList<Integer> leikkausJoukoilla(ArrayList<Integer> A, ArrayList<Integer> B) {
        return new ArrayList<>(new TreeSet<>() {{ addAll(A); retainAll(B); }});
    }
