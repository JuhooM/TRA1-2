...
import fi.uef.cs.tra.GraphException;
...
public class TRAII_22s_t19_20_pohja {
    ...
    static void taydennaYhtenaiseksi(Graph G) {
        ...
        for(Vertex v : G.vertices()) {
            if(v.getColor() == Graph.BLACK)
                continue;
            else if(v.getColor() != Graph.BLACK && edellinen != null)
                v.addEdge(edellinen);
    ...
    static void taydenna2yhtenaiseksi(Graph G) {
        ...
        // TODO
        List<Vertex> x = leikkausSolmut(G);
        for(Vertex v : x) {
            Vertex edellinen = null;
            for(Vertex ve : v.neighbors()) {
                if(edellinen != null)
                    edellinen.addEdge(ve);
                edellinen = ve;
            }
        }
    }
