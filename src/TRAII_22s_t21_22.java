...
public class TRAII_22s_t21_22 implements  DoubleSetFile, Iterable<Double> {
     ...
     */
    public TRAII_22s_t21_22(String tiedostonNimi, int koko) {
        hajautusKoko = koko;
        try {
            synchronized (this) {
     ...
     */
    public TRAII_22s_t21_22(String tiedostonNimi) {
        try {
            synchronized (this) {
                tiedosto = new RandomAccessFile(tiedostonNimi, "rw");
    ...
    public boolean add(Double x) {
        ...
        try {
            tiedosto.seek(otsikkoPituus + kotiOsoite * tietueKoko);
            for (int i = 0; i < Math.floor(ylivuotoTila/tietueKoko); i++) {
                // TODO
                // etsitÃ¤Ã¤n onko x luetussa tavutaulukossa (jolloin ei lisÃ¤tÃ¤)
                if(tiedosto.readDouble() == x)
                    return false;
            }  
            ...
            for (int i = 0; i < Math.floor(ylivuotoTila/tietueKoko); i++) {
                ...
                if(tiedosto.readDouble() == 0) {
                    tiedosto.writeDouble(x);
                    return true;
                }
            }  
